...
3. **Wait for CI**: poll `gh pr view <N> --json statusCheckRollup` until all
   ...
   Master/Worker E2E. A release PR to `master` also runs the Release Readiness
   check, which fails when the batch has no bump-worthy Conventional Commit and
   would therefore not produce a tag.
...
- [ ] CI keeps separate validation, smoke, package-surface, init-install, and
  ...
  helper split.
- [ ] `.github/workflows/release.yml` runs on `v*.*.*` tag push (created by
  auto-tag.yml) and publishes immutable version tags only after locked sync,
  setup, validation, smoke tests, release-style build, artifact inspection,
  installer preparation, SHA256 checksum generation, GitHub Artifact
  Attestations, isolated wheel-install smoke testing, and
  `gh release create` with changelog generated by git-cliff.
- [ ] Release artifacts carry GitHub Artifact Attestations
  (`actions/attest-build-provenance`) so users can verify provenance with
  `gh attestation verify <artifact> --repo EraserandRain/envmgr`.
...
- [ ] Release notes prepend fixed install, SHA256 verification, upgrade,
  uninstall, and clean-reinstall guidance before the git-cliff changelog.
